// Coger Admin Dashboard v2.0 - Enhanced Implementation
// Features: Error handling, search/filter, better data management

const db = {
  products: [
    {id: 1, name: 'Coconut Oil Premium', sku: 'COO-001', category: 'Oils', price: 450, stock: 150, status: 'Active'},
    {id: 2, name: 'Desiccated Coconut', sku: 'DES-002', category: 'Powders', price: 280, stock: 15, status: 'Low Stock'},
    {id: 3, name: 'Coconut Milk 200ml', sku: 'MIL-003', category: 'Beverages', price: 120, stock: 2, status: 'Out of Stock'}
  ],
  orders: [
  ...
  customers: [
    {id: 1, name: 'Raj Kumar', email: 'raj@email.com', phone: '9876543210', orders: 5, spent: 12500, status: 'Active'},
    {id: 2, name: 'Priya Singh', email: 'priya@email.com', phone: '9876543211', orders: 3, spent: 7800, status: 'Active'},
    {id: 3, name: 'Amit Patel', email: 'amit@email.com', phone: '9876543212', orders: 8, spent: 25400, status: 'VIP'}
  ],
  reviews: [
    {id: 1, customer: 'Raj', product: 'Coconut Oil', rating: 5, text: 'Excellent quality!', date: '2025-12-04', status: 'Approved'},
...
function refreshDash() {
  ...
  } catch (e) { console.log('Dashboard metrics not found'); }
}
  ...
  try {
    list.innerHTML = `
      <table class="data-table">
        <thead>
          <tr>
            <th>Product</th><th>SKU</th><th>Category</th><th>Price</th><th>Stock</th><th>Status</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          ${db.products.map(p => `
        ...
        <tbody>
          ${db.orders.map(o => `
            <tr>
              ...
              <td>${o.payment}</td>
              <td><button class="btn btn-small" onclick="viewOrder('${o.id}')">View</button></td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    `;
  } catch (e) { showError('Failed to load orders'); }
}
  ...
  try {
    list.innerHTML = `
      <table class="data-table">
        <thead>
          <tr>
            <th>Name</th><th>Email</th><th>Phone</th><th>Orders</th><th>Total Spent</th><th>Status</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          ${db.customers.map(c => `
            <tr>
              ...
              <td><span class="badge badge-${c.status === 'VIP' ? 'success' : 'info'}">${c.status}</span></td>
    ...
    `;
  } catch (e) { showError('Failed to load customers'); }
}
...
function loadReviews() {
  ...
  try {
    ...
    `;
  } catch (e) { showError('Failed to load reviews'); }
}

function editProduct(id) { showToast('Edit product ' + id + ' - Feature ready', 'info'); }
...
function approveReview(id) {
  ...
  if (r) { r.status = 'Approved'; loadReviews(); showToast('Review approved', 'success'); }
}
